...
void CNoteEffect::Update( const float& DeltaTime )
{
	...
	// 現在のアニメーションの位置の更新.
	m_NowAnimPoint += 180.0f * DeltaTime;

	// アニメーションが終了したか.
	if ( m_NowAnimPoint >= 180.0f ) {
		m_NowAnimPoint	= 180.0f;
		m_IsDisp		= false;
		m_IsAnimEnd		= true;
	}

	// 音符を揺らす.
	m_SpriteState.Transform.Rotation.z = Math::ToRadian( 20.0f ) * sinf( Math::ToRadian( m_NowAnimPoint * 2.0f ) );

	// 透明度の設定.
	m_SpriteState.Color.w = sinf( Math::ToRadian( m_NowAnimPoint ) );
}
