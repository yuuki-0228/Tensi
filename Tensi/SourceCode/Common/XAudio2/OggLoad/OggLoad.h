...
public:
	...
	// oggデータ作成.
	HRESULT CreateOggData( std::string sFilePath, std::string sFileName, const bool& isSE = false, bool isBuffered = false );
	// ファイルを開く
	HRESULT Open( std::string sFilePath, std::string sFileName, bool isBuffered = false );
	// ファイルを閉じる
	HRESULT Close();
	// ファイルを読み込む
